...
static nx_err_t rl_init(nx_lora_radio_t *radio, const nx_lora_config_t *config)
{
    ...
    /* DIO2-as-RF-switch: bare SX1262 modules (Heltec V3) use DIO2 to drive
     * the chip's internal T/R select.  Boards with a discrete external
     * RXEN/TXEN switch (RAK4631, XIAO nRF52840 + WIO-SX1262, XIAO ESP32S3
     * + WIO-SX1262) set -DNX_LORA_RF_SWITCH_EXTERNAL=1 and install their
     * own RadioLib switch table from main.cpp.  Gate on the build flag so
